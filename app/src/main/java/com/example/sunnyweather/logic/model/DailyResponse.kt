...
data class DailyResponse(val status: String, val forecasts: List<Forecast>,val lifeIndex: LifeIndex) {

    data class Forecast(val casts: List<Cast>)
    data class Cast(val date: Date,val daytemp: Float,val nighttemp: Float,val dayweather: String,val nightweather: String)
